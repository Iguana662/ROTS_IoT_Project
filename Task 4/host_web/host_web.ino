  ...
  <head>
    ...
    <style>
      body {
        ...
        /* Default color, but will be overridden by JS immediately */
        background: #f2f3f5; 
        color: #333;
        text-align: center;
        margin: 0;
      ...
      .container {
        /* Made slightly more transparent to blend with changing background */
      ...
      .sensor { font-size: 1.1em; margin: 10px 0; }
      ...
      #settings:hover { background: #e6f3ff; }
    </style>
  </head>
    ...
    <script>
      ...
      function updateBackgroundColor(t) {
        ...
        if (t < 20) {
           ...
           // Standard Blue (Sky Blue)
           body.style.backgroundColor = "#90CAF9"; 
        } else {
           // Orange (Warm)
      ...
      setInterval(()=>{
        fetch('/sensors')
          ...
          .then(d=>{
            ...
            // Call the color update function with new temperature
            updateBackgroundColor(d.temp);
          });
      },3000);

      // Run immediately on page load to set initial color
      window.onload = function() {
        let initialTemp = parseFloat(document.getElementById('temp').innerText);
        updateBackgroundColor(initialTemp);
      };
    </script>
  </body>
  </html>
...
void setup() {
  ...
  xTaskCreate(main_server_task, "server_task", 4096, NULL, 1, NULL);
}
